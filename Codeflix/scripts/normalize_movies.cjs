...
try {
  ...
  const normalized = data.map(m => {
    ...
    movie.miedo = Number(
      (typeof movie.miedo === 'number' && movie.miedo >= 0) ? movie.miedo :
      (typeof movie.scares === 'number' ? movie.scares :
        (typeof movie.scare === 'number' ? movie.scare : 0))
    );

    // jumps -> preferir 'jumps' o 'jumpscares'
    movie.jumps = Number(
      (typeof movie.jumps === 'number') ? movie.jumps :
      (typeof movie.jumpscares === 'number' ? movie.jumpscares : 0)
    );

    // suspenso -> preferir 'suspenso' o 'suspense' o fallback a 'scares'
    movie.suspenso = Number(
      (typeof movie.suspenso === 'number' ? movie.suspenso :
        (typeof movie.suspense === 'number' ? movie.suspense :
          (typeof movie.scares === 'number' ? movie.scares : 0))))
    ;
